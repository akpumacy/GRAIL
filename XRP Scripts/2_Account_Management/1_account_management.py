...
class XRPLMonitorThread(Thread):
    ...
    async def on_connected(self):
        ...
        if self.wallet:
            ...
            #     client=self.client,
            # )
            # response = await xrpl.asyncio.transaction.send_reliable_submission(hst_prepared, self.client)
    ...
    async def send_token(self, paydata):
        """
        Prepare, sign, and send an XRP payment with the provided parameters.
        Expects a dictionary with:
        {
            "currcode": Currency Code
            "coldw": Cold Wallet Address
            "to": Destination address (classic or X-address)
            "amt": Amount of decimal XRP to send, as a string
        }
        """
        currency_code = paydata.get("currcode", "")
        to_address = paydata["to"]
        cold_wallet_address = paydata["coldw"]
        issue_quantity = paydata["amt"]
        ...
        print(f"Sending {issue_quantity} {currency_code} to {to_address}...")
        response = await xrpl.asyncio.transaction.send_reliable_submission(pay_prepared, self.client)

        wx.CallAfter(self.gui.add_pending_tx, pay_prepared)
...
class SendTokenDialog(wx.Dialog):
    """
    Pop-up dialog that prompts the user for the information necessary to send a
    direct XRP-to-XRP payment on the XRPL.
    """
    def __init__(self, parent, max_send=100000000.0):
        ...
        bmp_err = wx.ArtProvider.GetBitmap(wx.ART_ERROR, wx.ART_CMN_DIALOG, size=(16,16))
        self.err_to = wx.StaticBitmap(self, bitmap=bmp_err)
        self.err_currcode = wx.StaticBitmap(self, bitmap=bmp_err)
        self.err_coldw = wx.StaticBitmap(self, bitmap=bmp_err)
        self.err_amt = wx.StaticBitmap(self, bitmap=bmp_err)
        self.err_to.Hide()
        self.err_currcode.Hide()
        self.err_amt.Hide()
        self.err_coldw.Hide()
        ...
        lbl_to = wx.StaticText(self, label="To (Address):")
        lbl_currcode = wx.StaticText(self, label="Token/Currency Code:")
        lbl_coldw = wx.StaticText(self, label="Cold wallet address:")
        lbl_amt = wx.StaticText(self, label="Token Amount:")
        self.txt_to = wx.TextCtrl(self, value="rMwHZCk7PHYgqLgNDyxpi9ysgeTPkLFRb")
        self.txt_currcode = wx.TextCtrl(self, value="RED")
        self.txt_coldw = wx.TextCtrl(self,value="rMCUMfSKYcuszuXh3uwoBvxe3WVmhkxB5a")
        self.txt_amt = wx.SpinCtrlDouble(self, value="5.0", min=0.000001, max=max_send)
        self.txt_amt.SetDigits(6)
        self.txt_amt.SetIncrement(1.0)
        ...
        sizer.BulkAdd(((lbl_to, self.txt_to, self.err_to),
                       (self.domain_verified, self.domain_text),
                       (lbl_currcode, self.txt_currcode, self.err_currcode),
                       (lbl_coldw, self.txt_coldw, self.err_coldw),
                       (lbl_amt, self.txt_amt, self.err_amt),
                       (btn_cancel, self.btn_send)) )
        sizer.Fit(self)
    ...
    def get_payment_data(self):
        ...
        return {
            "to": self.txt_to.GetValue().strip(),
            "currcode": self.txt_currcode.GetValue().strip(),
            "coldw":self.txt_coldw.GetValue().strip(),
            "amt": self.txt_amt.GetValue(),
        }
    ...
    def on_to_edit(self, event):
        ...
        if xrpl.core.addresscodec.is_valid_xaddress(v):
            ...
            if cl_addr == self.parent.classic_address:
                err_msg = "Can't send Token to self."
            elif is_test != self.parent.test_network:
                err_msg = "This address is intended for a different network."
...
class TWaXLFrame(wx.Frame):
    ...
    def build_ui(self):
        ...
        self.ttx = wx.Button(main_panel, label="Send Token")
        self.ttx.SetToolTip("Disabled in read-only mode.")
        self.ttx.Disable()
        self.Bind(wx.EVT_BUTTON, self.click_send_token, source=self.ttx)
        ...
        main_sizer.Add(self.ttx, 0, flag=wx.ALL, border=5)
    ...
    def prompt_for_account(self):
        ...
        account_dialog = wx.TextEntryDialog(self,
                ...
                value="sEdV5uQcY3iayMv1hgs45CFS6fhzjhG")
    ...
    def click_send_token(self,event):
        dlg = SendTokenDialog(self, max_send=float(0))
        dlg.CenterOnScreen()
        resp = dlg.ShowModal()
        if resp != wx.ID_OK:
            print(f"Sending canceled")
            dlg.Destroy()
            return

        paydata = dlg.get_payment_data()
        dlg.Destroy()
        self.run_bg_job(self.worker.send_token(paydata))
        notif = wx.adv.NotificationMessage(title="Sending!", message =
                f"Sending a payment for {paydata['amt']} {paydata['currcode']}!")
        notif.SetFlags(wx.ICON_INFORMATION)
        notif.Show()
